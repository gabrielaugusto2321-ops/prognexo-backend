...
export async function handleCampaignSendJob(job, {
  ...
  try {
    ...
    const [{ data: campaign }, { data: lead }, integration] = await Promise.all([
      ...
      client.from('leads').select('id,doctor_id,telefone_normalizado,whatsapp_authorization_status').eq('id', leadId).maybeSingle(),
    ...
    else if (!normalizeBrazilianPhone(lead.telefone_normalizado).valid) blockedStatus = 'telefone_invalido';
    if (blockedStatus) {
    ...
    else if (!normalizeBrazilianPhone(freshLead.telefone_normalizado).valid) finalBlock = 'telefone_invalido';
    if (finalBlock) {
    ...
    await send(integration.external_id, integration.access_token, freshLead.telefone_normalizado, campaign.mensagem);
