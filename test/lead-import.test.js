...
describe('normalizeBrazilianPhone', () => {
  it.each([
    ['11987654321', '5511987654321'], ['5511987654321', '5511987654321'],
    ['1133334444', '551133334444'], ['551133334444', '551133334444'],
    ['+55 (11) 98765-4321', '5511987654321'],
  ])('normaliza %s', (input, expected) => expect(normalizeBrazilianPhone(input)).toEqual({ valid: true, normalized: expected, reason: null }));
  ...
  it('mantém o nono dígito intacto (não inventa nem remove)', () => {
    expect(normalizeBrazilianPhone('11987654321').normalized).toBe('5511987654321'); // 11 dígitos, com 9
    expect(normalizeBrazilianPhone('1133334444').normalized).toBe('551133334444'); // 10 dígitos, sem 9
  });
});
...
describe('CSV parser', () => {
  ...
  it('autorização "sim" sem data válida é sempre inválida (telefones diferentes, sem interferência de duplicidade)', async () => {
    const result = await processImportFile(Buffer.from(csv('A;11987654321;;;sim;', 'B;11888888888;;;nao;')), { fetchExistingPhoneMap: async () => new Map() });
...
describe('lead import endpoints', () => {
  it('preview não escreve e identifica existente apenas no mesmo médico', async () => {
    db.tables.leads.push({ id: 'lead-a', doctor_id: doctorA, telefone_normalizado: '5511987654321' }, { id: 'lead-b', doctor_id: doctorB, telefone_normalizado: '5511888888888' });
    ...
    const res = await request(app).post(`/leads/import/preview?doctor_id=${doctorA}`).set(auth('a')).set('Content-Type', 'text/csv')
      .send(csv('A;11987654321;;;nao;', 'B;11888888888;;;nao;'));
  ...
  it('commit é idempotente e não reverte opt_out', async () => {
    ...
    const body = csv('A;11987654321;Feira;Joao;sim;2025-01-01', 'B;11888888888;;;nao;');
    const url = `/leads/import/commit?doctor_id=${doctorA}&nome_lista=Lista&filename=x.csv`;
  ...
  describe('correção 4 — retry de falha parcial', () => {
    const url = `/leads/import/commit?doctor_id=${doctorA}&nome_lista=Lista&filename=x.csv`;
    const body = csv('A;11987654321;;;nao;', 'B;11888888888;;;nao;');
